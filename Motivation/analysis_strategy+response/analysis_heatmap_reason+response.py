...
def strict_segmentation(metrics):
    """
    严格切分 metrics:
    1. Reasoning: <reasoning> 标签之后 -> </reasoning> 标签之前
    2. Response: <response> 标签之后 -> </response> 标签之前
    """
    # 将所有token组合成文本，方便查找
    ...
    if reasoning_start != -1 and reasoning_end != -1:
        ...
        if start_index < end_index:
            reasoning_metrics = metrics[start_index:end_index]
    ...
    return reasoning_metrics, response_metrics
...
def generate_html_file(samples, filename_base, title, output_dir,remark=""):
    ...
    filename = f"{name_part}{suffix}{ext}" # <--- 新的文件名
...
def process_and_generate(data, output_dir,remark=""):
    ...
    for session_idx, session in enumerate(data):
        ...
        for turn in history:
            if turn['role'] == 'Persuader':
                ...
                if not raw_metrics: continue # 跳过无 metrics 数据
                
                strategy_name = turn.get('strategy', 'Unknown')
                
                # === 切分 ===
                m_reasoning, m_response = strict_segmentation(raw_metrics)
                
                # 存入 Reasoning 列表 (前提是切分出来不为空)
                if len(m_reasoning) > 5:
    ...
    # 生成两个独立文件
    print(f"Generating Reasoning Heatmap ({len(reasoning_samples)} samples)...")
    generate_html_file(reasoning_samples, "analysis_reasoning_heatmap.html", "Reasoning Process Entropy", output_dir, remark)
