...
class RolloutManager:
    ...
    def _generate_distinct_step(self, context, existing_strategies,
        ...
        for i in range(max_retries):
            ...
            if i >= 2 and existing_strategies:
...
def main():
    ...
    target_users = EXP_ALL_USERS # 导入实验用户集合

    version = "v1"  # 版本号，可根据需要修改
    output_file = os.path.join(args.input_dir, f"rollout_{version}_t_{args.branching_temperature}_thinking_{args.enable_thinking}/results_{args.mode}_t_{args.branching_temperature}.json")
